"""The session factory, and the guard that keeps an unsupported JDK from
turning into a stack trace nobody can read."""
...
def test_unsupported_java_fails_with_an_actionable_message(monkeypatch) -> None:
...
def test_undetectable_java_does_not_block(monkeypatch) -> None:
    """Better to let Spark report the problem than to refuse on a failed probe."""
...
def test_path_is_used_when_java_home_points_nowhere(monkeypatch, tmp_path: Path) -> None:
